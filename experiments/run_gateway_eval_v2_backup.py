...
BASE_CASE_FILE = PROJECT_ROOT / "security_cases" / "gateway_cases.json"
V2_CASE_FILE = PROJECT_ROOT / "security_cases" / "gateway_cases_v2.json"
...
def load_cases() -> List[Dict[str, Any]]:
    ...
    for path in [BASE_CASE_FILE, V2_CASE_FILE]:
        if path.exists():
...
def run_eval() -> Dict[str, Any]:
    ...
    if not cases:
        raise RuntimeError("没有读取到任何测试样例，请检查 security_cases 目录。")
...
def write_outputs(rows: List[Dict[str, Any]], summary: Dict[str, Any]) -> None:
    ...
    if not summary["failed_cases"]:
        ...
        for item in summary["failed_cases"]:
            md.append(
                f"- {item['id']}：expected={item['expected']}，actual={item['actual']}，risk_score={item['risk_score']}\n"
            )
