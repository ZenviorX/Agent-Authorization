...
Agent-Authorization（展示名：AgentGuard）是一个面向 AI Agent 工具调用场景的授权与安全防护系统。项目聚焦于大语言模型 Agent 在调用文件、邮件、数据库、Shell 命令等外部工具时可能产生的越权访问、提示注入、敏感信息泄露、危险命令执行和多步攻击链风险，设计并实现了一套“授权检查—风险评估—能力约束—沙箱执行—审计取证”的安全闭环。
...
> 不把 Agent 当成完全可信主体，而是将每一次工具调用都放入任务边界、能力合约、风险评分、攻击链上下文和审计证据中进行动态授权。
...
这类能力显著提升了 Agent 的实用性，但也带来了新的安全风险。一旦 Agent 被提示注入诱导，或者工具权限边界不清晰，就可能出现以下问题：
...
4. **高危工具调用**：Agent 可能执行 `rm -rf`、`shutdown`、`curl`、`wget`、`DROP TABLE` 等危险操作。
...
Gateway 会根据用户角色、工具类型、资源路径、参数内容、风险策略、任务合约和攻击链状态输出最终决策。

---

### 3.2 策略化风险评分

项目使用 `config/policy.yaml` 作为统一策略配置文件。当前策略版本为 v3，系统只保留两类角色：
...
这样可以让用户和评审清楚看到：为什么放行、为什么需要确认、为什么拒绝，以及风险主要来自路径、工具、命令、SQL、邮件外发还是任务合约。

---

### 3.4 Capability Contract 任务能力合约
...
| 角色 | 权限设计 |
|---|---|
| `user` | 允许读取 public/course，允许普通数据库查询，写 public、发邮件、删除 public 文件需要确认，禁止访问 secret/private/credentials，禁止执行 shell.run |
| `admin` | 可以访问更多资源，但删除文件和 shell.run 仍然需要确认，高风险敏感路径仍会被风险分拉高 |
...
2. **强化 CI 门禁**：将单元测试、安全评测、攻击链评测和报告生成拆分为严格 CI 阶段。
3. **增强 Runtime 持久化**：将运行时任务状态从内存进一步扩展为 JSONL、SQLite 或 Redis。
4. **接入真实 LLM Agent**：在保留 FakeAgent 演示的基础上，引入真实 LLM 工具调用流程。
5. **MCP 工具代理适配**：将 Gateway 封装为 MCP 工具调用前置代理，保护文件系统、数据库、浏览器等 MCP 工具。
6. **策略可视化配置**：提供策略编辑页面，使用户可以更直观地配置风险阈值、角色权限和任务边界。
7. **更完整的对比实验**：对比无防护 Agent、关键词过滤器、单步 Gateway 和本系统的攻击阻断效果。
